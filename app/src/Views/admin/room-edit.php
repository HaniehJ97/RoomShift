...
<div class="admin-container">
    ...
    <div class="admin-content">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2>Edit Room: <?= htmlspecialchars($room->title) ?></h2>
            <div>
                <span class="badge bg-<?= 
        ...
        <div class="card">
            <div class="card-body">
                <form method="POST" action="/admin/rooms/<?= $room->id ?>/edit">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="description" name="description" 
                                  rows="3" required><?= htmlspecialchars($room->description) ?></textarea>
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control" id="estimated_time" name="estimated_time" 
                                   value="<?= $room->estimated_time ?>" min="1" max="180">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Publication Status</label>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="is_published" 
                                       name="is_published" value="1" <?= $room->is_published ? 'checked' : '' ?>>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="card bg-dark">
                            <div class="card-body">
                                ...
                                <input type="hidden" id="levelConfigJson" name="config_json" 
                                       value='<?= json_encode($room->level_config) ?>'>
                                ...
                                <div class="row mt-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" class="form-control" id="gridWidth" 
                                               value="<?= $room->level_config['grid_width'] ?? 12 ?>" min="8" max="30">
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">Grid Height</label>
                                        <input type="number" class="form-control" id="gridHeight" 
                                               value="<?= $room->level_config['grid_height'] ?? 12 ?>" min="8" max="30">
                                    </div>
                                </div>
                                
                                <input type="hidden" name="grid_width" id="gridWidthInput" value="<?= $room->level_config['grid_width'] ?? 12 ?>">
                                <input type="hidden" name="grid_height" id="gridHeightInput" value="<?= $room->level_config['grid_height'] ?? 12 ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <div>
                            <a href="/rooms/<?= $room->id ?>/play" class="btn btn-outline-info me-2" target="_blank">
                                <i class="bi bi-play me-2"></i>Test Room
                            </a>
                            <button type="button" class="btn btn-outline-danger" 
                                    onclick="if(confirm('Delete this room?')) document.getElementById('deleteForm').submit();">
                ...
                <!-- Delete Form -->
                <form method="POST" action="/admin/rooms/<?= $room->id ?>/delete" id="deleteForm" class="d-none">
                </form>
            </div>
        </div>
...
window.levelEditorData = {
    gridWidth: <?= $room->level_config['grid_width'] ?? 12 ?>,
    gridHeight: <?= $room->level_config['grid_height'] ?? 12 ?>,
    config: <?= json_encode($room->level_config) ?>
};
</script>
